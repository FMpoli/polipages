<section style="background-color: {{ $background_color }}">
    <div class="max-w-screen-xl px-4 py-8 mx-auto sm:py-16 lg:px-6">
        <h2 class="mb-8 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">{{ $title }}</h2>
        <div class="grid pt-8 text-left border-t border-gray-200 md:gap-16 dark:border-gray-700 md:grid-cols-2">
            <div>
                @php
                    $faqCount = count($faq);
                    $halfCount = ceil($faqCount / 2); // Divide il conteggio delle faq per due e arrotonda verso l'alto
                    $firstColumn = array_slice($faq, 0, $halfCount); // Prima metà delle faq
                    $secondColumn = array_slice($faq, $halfCount); // Seconda metà delle faq
                @endphp
